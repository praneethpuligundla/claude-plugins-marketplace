...
func TestLoadContextState(t *testing.T) {
	...
	t.Run("same session ID loads existing state", func(t *testing.T) {
		...
		if state.TotalToolCalls != 10 {
			t.Errorf("TotalToolCalls = %v, want 10", state.TotalToolCalls)
		}
	})
}
...
func TestAddEntry(t *testing.T) {
	...
	t.Run("simple counting (no token estimation)", func(t *testing.T) {
		state := &ContextState{SessionID: "test"}

		// Add several entries
		state.AddEntry("Read", "short")
		state.AddEntry("Read", "content")
		state.AddEntry("Bash", "output")

		// Only counting, no complex estimation
		if state.TotalToolCalls != 3 {
			t.Errorf("TotalToolCalls = %v, want 3", state.TotalToolCalls)
		}
	})
}

func TestNeedsCompaction(t *testing.T) {
	// NeedsCompaction now always returns false (philosophy change)
	state := &ContextState{UtilizationPercent: 0.9}
	if got := state.NeedsCompaction(0.7); got != false {
		t.Errorf("NeedsCompaction() = %v, want false (always)", got)
	}
}
...
func TestGetSummary(t *testing.T) {
	...
	if !strings.Contains(summary, "Compactions: 2") {
		t.Errorf("Summary should contain 'Compactions: 2', got: %v", summary)
	}
}
